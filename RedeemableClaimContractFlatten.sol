...
// File contracts/RedeemableClaim.sol
...
/*
    ERC20 contract where balances represent the number of claims an address
    has the right to redeeem at the future date per the terms of service.
    Note that redeeming a claim wll always required some level of verification, i.e. KYC / KYB,
    as defined in the terms of service associated with the sale. 
*/
contract RedeemableClaim is ERC20, ERC20Pausable, AccessControl, ERC20Permit {
    ...
    function pause() public payable onlyRole(PAUSER_ROLE) {
        _pause();
    }

    function unpause() public payable onlyRole(PAUSER_ROLE) {
        _unpause();
    }

    // Adds additional claims
    function allocate(address to, uint256 amount) public onlyRole(MINTER_ROLE) {
        _mint(address(to), amount);
    }

    // Add an address to process redemptions
    function addRedeemer(address redeemer) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(REDEEMER_ROLE, redeemer);
    }

    // Remove an address from processing redemptions
    function removeRedeemer(address redeemer) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _revokeRole(REDEEMER_ROLE, redeemer);
    }

    // Removes claims from caller and supply, representing claims redeemed
    function redeem(uint256 amount) public onlyRole(REDEEMER_ROLE) {
